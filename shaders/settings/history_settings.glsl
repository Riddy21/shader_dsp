uniform sampler2D audio_history_texture_old;

int get_audio_history_size() {
    ivec2 audio_size = textureSize(audio_history_texture_old, 0);
...
float get_audio_history_sample(int index, int channel) {
    ...
    return texture(audio_history_texture_old, vec2(index_real) / vec2(audio_size)).r;
}