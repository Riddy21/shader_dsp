...
void main() {
    ...
    for (int i = 1; i <= num_echos; i++) {
        ...
        float echo_sample_index = float(i) * delay_increment * float(num_channels) + channel_increment + epsilon;
        float decay_factor = pow(decay, float(i));
        // TODO: Figure out a way to make echo sound so robotic
        if (echo_sample_index >= 1.0) {
            break;
        }
        vec2 echo_sample_coord = vec2(TexCoord.x, echo_sample_index);
        // WIP still working on the echo
