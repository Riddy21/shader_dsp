...
TEST_CASE("AudioTexture2DParameter with OpenGL context", "[audio_parameter][gl_test]") {
    ...
    SDL_Window* window = SDL_CreateWindow(
        ...
        256, 64, SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN
    );
    ...
    // --- Use AudioShaderProgram for shader compilation ---
    AudioShaderProgram shader_prog(vert_src, frag_src);
    REQUIRE(shader_prog.initialize());
    GLuint prog = shader_prog.get_program();
    ...
    // --- Create OUTPUT AudioTexture2DParameter ---
    AudioTexture2DParameter output_param(
        "color",
        AudioParameter::ConnectionType::OUTPUT,
        width, height,
        0, // active_texture (not used for OUTPUT)
        0, // color_attachment
        GL_NEAREST,
        GL_FLOAT,
        GL_RGBA,
    ...
    REQUIRE(output_param.initialize(fbo, &shader_prog));
    ...
    output_param.render(); // Render the parameter to initialize it
    ...
    // --- Read back using AudioTexture2DParameter API ---
    const float* pixels = static_cast<const float*>(output_param.get_value());

    // --- Verify sine wave pattern ---
    for (int i = 0; i < width; ++i) {
        float expectedRed = sin(static_cast<float>(i) / width * 2.0f * M_PI);
        float actualRed = pixels[i * 4 + 0];
        REQUIRE(actualRed == Catch::Approx(expectedRed).margin(0.05f));
        REQUIRE(pixels[i * 4 + 1] == 0.0f); // Green channel should be 0
        REQUIRE(pixels[i * 4 + 2] == 0.0f); // Blue channel should be 0
        REQUIRE(pixels[i * 4 + 3] == 1.0f); // Alpha channel should be 1
    }
